...
class ActiveFire:
    ...
    def VIIRS_NOAA2(self):
        from qgis.core import QgsApplication
        import requests
        openvirs = 'https://firms.modaps.eosdis.nasa.gov/data/active_fire/noaa-21-viirs-c2/shapes/zips/J2_VIIRS_C2_Global_24h.zip'  # link to VIIRS download
        r = requests.get(openvirs, allow_redirects=True)
        pluginpath = QgsApplication.qgisSettingsDirPath() + r'\python\plugins\active_fire'
        open(pluginpath + r'\VIIRS_NOAA224.zip', 'wb').write(r.content)
        self.iface.addVectorLayer(pluginpath + r'\VIIRS_NOAA224.zip', 'VIIRS NOAA2 24H', 'ogr')
        a = self.iface.activeLayer()
        ren = a.renderer()
        sym = QgsMarkerSymbol.createSimple(
            {'name': 'square', 'color': '#860415', 'size': '0.8', 'outline_width': '0', 'outline_color': '#860415'})
        ...
        a.triggerRepaint()  # Refresh Layer
        self.iface.layerTreeView().refreshLayerSymbology(a.id())
        a.setName(a.name().replace(a.name(), 'VIIRS 375m/NOAA-21'))
        self.dlg.lineEdit_4.setText(str(a.featureCount()))
    ...
    def VIIRS_NOAA21(self):
        from qgis.core import QgsProject
        if not QgsProject.instance().mapLayersByName('VIIRS 375m/NOAA-21'):
            self.VIIRS_NOAA2()
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.pushButton_5.clicked.connect(self.VIIRS_NOAA21)
